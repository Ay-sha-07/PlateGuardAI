...
/* -------------------- Scroll-driven phone scan reveal -------------------- */

function ScrollScanReveal() {
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      raf = requestAnimationFrame(() => {
        ...
        // How far the section has been scrolled through (0 when top hits viewport top, 1 when bottom leaves)
  ...
  // 0.00–0.25  phone fades / scales in, food visible
  // 0.20–0.55  green scanner line sweeps top → bottom
  // 0.50–0.75  result overlay fades + slides up
  // 0.75–1.00  hold the completed result

  const phoneOpacity = Math.min(1, progress / 0.18);
  const phoneScale = 0.88 + Math.min(1, progress / 0.2) * 0.12;

  // Scanner line position: 0 at top, 1 at bottom
  const scanT = Math.max(0, Math.min(1, (progress - 0.18) / 0.38));
  const scanVisible = progress > 0.15 && progress < 0.62;

  // Result reveal
  const resultT = Math.max(0, Math.min(1, (progress - 0.52) / 0.28));
  const resultOpacity = resultT;
  const resultY = (1 - resultT) * 28; // px

  return (
    <section
      ref={sectionRef}
      className="relative bg-gradient-to-b from-background via-primary/5 to-background"
      style={{ height: "320vh" }}
      aria-label="Interactive scan demonstration"
    >
      {/* Sticky stage */}
      <div className="sticky top-0 flex h-screen flex-col items-center justify-center overflow-hidden px-4">
        <div className="mb-6 text-center">
          ...
          <h2 className="mt-2 font-display text-2xl font-bold tracking-tight sm:text-3xl">
            Watch a plate turn into a verdict
          </h2>
          <p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">
            Scroll slowly — the phone does the rest.
          </p>
        </div>
        ...
        >
          {/* Outer bezel */}
          <div className="relative rounded-[2.6rem] border-[10px] border-zinc-900 bg-zinc-900 shadow-2xl ring-1 ring-black/40">
            {/* Dynamic island / notch */}
            <div className="absolute left-1/2 top-2.5 z-30 h-6 w-24 -translate-x-1/2 rounded-full bg-black" />

            {/* Screen */}
            <div className="relative aspect-[9/19] overflow-hidden rounded-[1.9rem] bg-black">
              {/* Food image (always present under everything) */}
              <img
                src="/media/hero-poster.jpg"
                alt="Plate of food being scanned"
                className="absolute inset-0 size-full object-cover"
                draggable={false}
              />

              {/* Subtle dark vignette so scan line & text read better */}
              ...
              {scanVisible && (
                <div
                  ...
                  style={{
                    ...
                    height: "3px",
                    background:
                      "linear-gradient(90deg, transparent 0%, oklch(0.72 0.19 155) 20%, oklch(0.85 0.22 145) 50%, oklch(0.72 0.19 155) 80%, transparent 100%)",
                    boxShadow:
                      "0 0 18px 4px oklch(0.72 0.19 155 / 0.7), 0 0 40px 8px oklch(0.65 0.18 150 / 0.35)",
                  }}
                >
                  {/* soft trail behind the line */}
                  <div
                    className="absolute inset-x-0 bottom-full h-16"
                    style={{
                      background:
                        "linear-gradient(to top, oklch(0.65 0.18 150 / 0.25), transparent)",
                    }}
                  />
                </div>
              )}

              {/* Scan grid overlay while scanning */}
              {scanVisible && (
                <div
                  className="pointer-events-none absolute inset-0 z-10 opacity-40"
                  style={{
                    backgroundImage: `
                      linear-gradient(oklch(0.7 0.18 155 / 0.15) 1px, transparent 1px),
                      linear-gradient(90deg, oklch(0.7 0.18 155 / 0.15) 1px, transparent 1px)
                    `,
                    backgroundSize: "28px 28px",
                    maskImage: `linear-gradient(to bottom, black ${Math.max(0, scanT * 100 - 8)}%, transparent ${scanT * 100}%)`,
                    WebkitMaskImage: `linear-gradient(to bottom, black ${Math.max(0, scanT * 100 - 8)}%, transparent ${scanT * 100}%)`,
                  }}
                />
              )}

              {/* Result overlay — styled after the third reference image, food still visible underneath */}
              <div
                className="absolute inset-0 z-30 flex flex-col"
                style={{
                  opacity: resultOpacity,
                  transform: `translateY(${resultY}px)`,
                  pointerEvents: resultOpacity > 0.5 ? "auto" : "none",
                }}
              >
                {/* Keep upper portion of the food image visible; dark panel rises from bottom */}
                <div className="absolute inset-0 bg-gradient-to-b from-transparent via-[#3d2b1f]/70 to-[#3d2b1f]/95" />
                <div className="absolute inset-x-0 bottom-0 top-[28%] bg-[#3d2b1f]/92 backdrop-blur-[2px]" />

                <div className="relative z-10 mt-auto flex w-full flex-col items-center px-4 pb-8 pt-6 text-center">
                  <div className="mb-3 flex size-14 items-center justify-center rounded-full bg-safe/20 ring-2 ring-safe/50">
                    <CheckCircle2 className="size-8 text-safe" strokeWidth={2.5} />
                  </div>
                  ...
                  <div className="mt-3 inline-flex items-center gap-1.5 rounded-full bg-safe px-4 py-1.5">
                    <CheckCircle2 className="size-3.5 text-safe-foreground" />
                    <span className="text-sm font-bold text-safe-foreground">Safe for you</span>
                  </div>
                  <button
                    type="button"
                    className="mt-5 w-full max-w-[200px] rounded-full border border-white/25 bg-white/10 py-2.5 text-sm font-semibold text-white backdrop-blur transition hover:bg-white/20"
                  >
                    View Details
                  </button>
                </div>
            ...
            <div className="absolute -left-[14px] top-24 h-8 w-[4px] rounded-l-sm bg-zinc-700" />
            <div className="absolute -left-[14px] top-36 h-12 w-[4px] rounded-l-sm bg-zinc-700" />
            <div className="absolute -right-[14px] top-28 h-14 w-[4px] rounded-r-sm bg-zinc-700" />
          </div>
        </div>
