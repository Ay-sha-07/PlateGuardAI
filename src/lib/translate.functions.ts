...
export const translateTexts = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    if (data.language === "en") return data.texts;
    const provider = getVisionProviders()[0];
    if (!provider) throw new Error("AI translation is unavailable because no AI provider is configured.");
    ...
    if (!Array.isArray(translated) || translated.length !== data.texts.length || !translated.every((x) => typeof x === "string")) throw new Error("AI returned an incomplete translation response.");
